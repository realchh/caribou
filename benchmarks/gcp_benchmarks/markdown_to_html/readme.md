...
The markdown_to_html benchmark converts a input Markdown file to HTML. It uses an Google Cloud Storage bucket for input and output file storage and is deployed using the Caribou framework.

The benchmark requires access to an Google Cloud Storage bucket named `markdown_to_html`. Alternatively, users can specify a different bucket name by modifying `gcp_bucket_name` in the app.py file.
