...
def plot_results(results: Dict[str, Dict[str, float]]):
    ...
    plt.tight_layout()
    filename = "t2s_workflow_latency_breakdown.png"
...
def analyze_scenarios():
    ...
    for name, window in SCENARIOS.items():
        ...
        for run_id, events in run_events.items():
            ...
            run_timeline = {}
            invocations = {}
            taint_to_trace = {}  # Map taint to the trace_id of the invoked function
            ...
            for event in events:
                ...
                if message.startswith("INVOKED"):
                    ...
                    if taint_match and trace_id:
                        taint_to_trace[taint_match.group(1)] = trace_id
                    ...
                    if taint_match and successor_match and invoked_match:
                        taint, successor, invoked = taint_match.group(1), successor_match.group(1).split(':')[
                            0], invoked_match.group(1) == 'True'
                        invocations[taint] = {'start_time': timestamp, 'from': instance_name, 'to': successor,
                                              'invoked': invoked}

            # Calculate delays using the new lookup table
            for taint, data in invocations.items():
                to_instance = data['to']
                if not data['invoked'] and to_instance == 'censor': continue

                # Use the taint to find the successor's trace ID
                successor_trace_id = taint_to_trace.get(taint)
                if successor_trace_id and successor_trace_id in trace_to_request_start:
                    ...
                    if delay > 0:
                        ...
                        if to_instance in run_timeline and 'start_time' in run_timeline[to_instance]:
                            platform_overhead = (
                                        run_timeline[to_instance]['start_time'] - request_start_time).total_seconds()
        ...
        print(f"  - Median Platform Overhead (censor): {median_results.get('median_platform_overhead_censor', 0):.4f}s")
