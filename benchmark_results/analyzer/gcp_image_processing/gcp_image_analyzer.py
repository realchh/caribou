...
def plot_results(results: Dict[str, Dict[str, float]]):
    ...
    ax.set_title('Workflow Latency Breakdown by Invocation Rate')
    ax.legend()
    ...
    plt.tight_layout()
    filename = "with5_workflow_latency_breakdown.png"
...
def analyze_scenarios():
    ...
    for name, window in SCENARIOS.items():
        ...
        if not logs_service_1 or not logs_service_2:
            print("Could not find logs for one or both services in this time window. Skipping scenario.")
            continue
        ...
        print(f"  - Average End-to-End Time: {avg_e2e:.4f}s")
        print(f"  - Average Pub/Sub Delay:   {avg_pubsub:.4f}s")
