...
SCENARIOS = {
    "Large Message (0.1 rps)": {
        "start_time": datetime(2025, 10, 14, 21, 29, 0, tzinfo=timezone.utc),
        "end_time": datetime(2025, 10, 14, 22, 14, 0, tzinfo=timezone.utc),
    },
    "Large Message (5 rps)": {
    ...
    "Small Message (0.1 rps)": {
        ...
        "end_time": datetime(2025, 10, 14, 21, 27, 0, tzinfo=timezone.utc),
    },
    "Small Message (5 rps)": {
...
def fetch_logs(client: logging_v2.Client, scenario_name: str, service_key: str, service_name: str, start_time: datetime,
    ...
    cache_filename = os.path.join(CACHE_DIR, f"http_{safe_scenario_name}_{service_key}.json")
...
def export_to_csv(all_metrics: Dict[str, Dict[str, list]]):
    ...
    output_filename = "http_latency_results.csv"
    print(f"\nExporting detailed results to {output_filename}...")

    fieldnames = [
        'scenario', 'message_size', 'invocation_rate', 'invocation_index',
        'total_http_latency_ms', 'request_latency_ms', 'transport_latency_ms'
    ]
        ...
        for scenario_name, metrics in all_metrics.items():
            ...
            for i in range(num_invocations):
                writer.writerow({
                    ...
                    'total_http_latency_ms': metrics['total_http_latency'][i] * 1000,
                    'request_latency_ms': metrics['request_latency'][i] * 1000,
...
def plot_results(all_metrics: Dict[str, Dict[str, list]]):
    ...
    for name, ax in scenario_map.items():
        ...
        if not metrics or 'total_http_latency' not in metrics:
            ax.text(0.5, 0.5, 'No data', ha='center', va='center')
            continue

        latencies_ms = np.array(metrics['total_http_latency']) * 1000.0
...
def analyze_scenarios():
    ...
    for name, window in SCENARIOS.items():
        ...
        for run_id, events in run_events.items():
            ...
            for event in events:
                ...
                if message.startswith("DEBUG_PUBLISHING_TO_MESSAGING_SERVICE"):
                    invocations[taint]['request_start_time'] = timestamp
                elif message.startswith("INVOKING_SUCCESSOR"):
                    invocations[taint]['request_sent_time'] = timestamp
            ...
            for taint, data in invocations.items():
                if all(k in data for k in ['request_start_time', 'request_sent_time', 'subscriber_invoked_time']):
                    subscriber_start_time = trace_to_request_start.get(data.get('subscriber_trace_id')) or data[
                        'subscriber_invoked_time']
                    total_latency = (subscriber_start_time - data['request_start_time']).total_seconds()
                    request_latency = (data['request_sent_time'] - data['request_start_time']).total_seconds()
                    transport_latency = (subscriber_start_time - data['request_sent_time']).total_seconds()
                    if total_latency >= 0 and request_latency >= 0:
                        metrics['total_http_latency'].append(total_latency)
                        metrics['request_latency'].append(request_latency)
                        metrics['transport_latency'].append(transport_latency)

        if not metrics['total_http_latency']:
        ...
        print(f"  - Found {len(metrics['total_http_latency'])} complete warm runs.")
        print(f"  - Median Total E2E Latency: {median_results.get('median_total_http_latency', 0):.4f}s")
        print("  --- Breakdown ---")
        print(
            f"    - Median Request Latency (Token Acq., etc.): {median_results.get('median_request_latency', 0):.4f}s")
        print(
            f"    - Median Transport Latency:                  {median_results.get('median_transport_latency', 0):.4f}s")
